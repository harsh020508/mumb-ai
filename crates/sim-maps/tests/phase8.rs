...
use sim_maps::{
    ...
    types::{tile_id_class, Grid, SemanticClass},
};
...
fn chunk_render_parallel(coords: &[(i32, i32)]) -> Vec<Grid<u32>> {
    coords
        .par_iter()
        .map(|&(cx, cy)| chunk_render_serial(cx, cy))
        .collect()
}
...
fn parallel_matches_serial_for_empty_features() {
    ...
    let serial: Vec<Grid<u32>> = coords
        .iter()
        .map(|&(cx, cy)| chunk_render_serial(cx, cy))
        .collect();
...
fn parallel_all_grass_chunks_are_uniform_grass() {
    let coords: Vec<(i32, i32)> = (0..2)
        .flat_map(|cy| (0..2).map(move |cx| (cx, cy)))
        .collect();
    let results = chunk_render_parallel(&coords);
...
fn parallel_slope_collision_consistent() {
    ...
    let results: Vec<Grid<u8>> = (0..8)
        .into_par_iter()
        .map(|_| semantic_to_collision(&sem, &rise, false, 1.5))
        .collect();
...
fn rayon_and_sequential_max_rise_match() {
    let elevations: Vec<Grid<f32>> = (0..8)
        .map(|i| Grid::from_fn(16, 16, |col, row| (col + row + i) as f32 * 0.1))
        .collect();
