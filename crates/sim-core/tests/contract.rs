...
async fn contract_all_endpoints() {
    ...
    let r_bad = c
        ...
        .json(&serde_json::json!({"city": "nonexistent_city"}))
        .send()
        .await
        .unwrap();
    ...
    let r_root = c
        ...
        .send()
        .await
        .expect("get root HTML");
    assert_eq!(r_root.status(), 200, "root HTML status");
    let html = r_root.text().await.unwrap();
    assert!(
    ...
    let r_api = c
        ...
        .await
        .expect("get api info");
    assert_eq!(r_api.status(), 200, "api status");
    let v_api: Value = r_api.json().await.unwrap();
    assert_eq!(v_api["service"], "mumb-ai");
    ...
    let v: Value = r.json().await.unwrap();
    assert!(
        v["cities"].as_array().map(|a| a.len()).unwrap_or(0) >= 5,
        "all 5 cities listed"
    );

    // ---- GET /cities/mumbai ----
    let r = c
        .get(format!("{base}/cities/mumbai"))
        .send()
        .await
        .expect("get city details");
    ...
    let r = c
        .post(format!("{base}/simulations"))
        .json(&serde_json::json!({"n": 1000, "seed": 42, "start_datetime": "2024-11-01T08:00:00Z", "tick_seconds": 30, "commit_every": 20}))
