...
pub fn build_population(
    ...
    build_population_with(records, n, seed, tiles, Arc::new(CityProfile::sf()))
}

/// Build a population for an explicit city profile (drives seeding + prompt context).
pub fn build_population_with(
    records: &[PumsRecord],
    n: usize,
    seed: u64,
    tiles: Option<&TilesDb>,
    profile: Arc<CityProfile>,
) -> Population {
...
fn make_agent(
    ...
    let (home, work) = if let Some(t) = tiles {
        ...
        let work = if rec.employed() {
            Some(sample_work_cell(t, &rec, rng, profile))
        } else {
            None
        };
        (home, work)
    ...
    // Richer lifestyle (hobbies/routine/spending), drawn LAST so every prior
    // demographic, value, name, and placement draw is byte-unchanged.
    let life = crate::lifestyle::generate(&rec, q, rng);
    persona.push_str(&lifestyle_sentence(&life));
...
/// Append a richer-lifestyle sentence to the persona prose (empty parts skipped).
fn lifestyle_sentence(life: &crate::lifestyle::Lifestyle) -> String {
    let mut parts: Vec<String> = Vec::new();
    if !life.routine.is_empty() {
        parts.push(format!("Day to day, they {}", life.routine));
    }
    if !life.hobbies.is_empty() {
        parts.push(format!("they enjoy {}", join_and(&life.hobbies)));
    }
    if !life.spending.is_empty() {
        parts.push(format!("and {}", life.spending));
    }
    if parts.is_empty() {
        return String::new();
    }
...
/// Value vector with city baselines, demographic deltas, and small seeded noise.
pub fn make_value_vector(
    ...
    let pol = &profile.politics;

    // city baselines (mean electorate lean); shared demographic deltas applied below
    let mut economic = pol.economic_base;
    let mut social = pol.social_base;
    let mut trust = pol.trust_base;
    let mut change = pol.change_base;
    ...
    // city baseline issue saliences; shared demographic deltas applied below
    let mut s_housing = pol.s_housing;
    let mut s_crime = pol.s_crime;
    let s_homeless = pol.s_homeless;
    let mut s_cost = pol.s_cost;
    let mut s_environment = pol.s_environment;
    let mut s_immigration = pol.s_immigration;
    if !homeowner {
        s_housing += 0.2;
        s_cost += 0.15;
    } else {
...
fn sample_work_cell(tiles: &TilesDb, rec: &PumsRecord, rng: &mut impl Rng, profile: &CityProfile) -> Cell {
    ...
    if rng.gen::<f64>() < profile.work.downtown_share {
        ...
        tiles.sample_residential_cell(profile.centroid(rec.puma), rng)
    }
}
