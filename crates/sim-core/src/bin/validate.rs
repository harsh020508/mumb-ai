...
use simfrancisco::persona::build_population_with;
...
fn parse_args() -> Args {
    ...
    while let Some(arg) = it.next() {
        match arg.as_str() {
            "--smoke" => a.smoke = true,
            "--quiet" => a.quiet = true,
            "--city" => a.city = it.next().unwrap_or(a.city.clone()),
            "--rubric" => a.rubric = it.next(),
...
async fn run(args: Args) -> i32 {
    ...
    let rubric = match Rubric::load(&rubric_path) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("failed to load rubric {rubric_path}: {e}");
            return 2;
        }
    };
    ...
    let profile = match CityProfile::load(&args.city) {
        Ok(p) => Arc::new(p),
        Err(e) => {
            eprintln!("failed to load city profile {} ({e})", args.city);
            return 2;
        }
    };
