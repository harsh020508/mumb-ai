...
impl Manifest {
    /// Northern-hemisphere UTM zone number parsed from the CRS string
    /// ("EPSG:326NN" -> NN). Defaults to 10 (SF) if unparseable.
    pub fn utm_zone(&self) -> f64 {
        self.crs
            .rsplit(':')
            .next()
            .and_then(|s| s.trim().parse::<i64>().ok())
            .map(|epsg| (epsg - 32600) as f64)
            .filter(|z| (1.0..=60.0).contains(z))
            .unwrap_or(10.0)
    }
}
...
impl TilesDb {
    ...
    pub fn cell_to_lonlat(&self, c: Cell) -> (f64, f64) {
        ...
        utm_to_lonlat(x, y, self.manifest.utm_zone())
    }
    ...
    /// Sample a walkable cell near a PUMA's grid centroid `(chunk_cx, chunk_cy, radius)`
    /// (supplied by the city profile, so this stays city-agnostic).
    pub fn sample_residential_cell(&self, centroid: (i64, i64, i64), rng: &mut impl Rng) -> Cell {
        let (ccx, ccy, radius) = centroid;
        let cpc = self.manifest.cells_per_chunk;
        for _ in 0..400 {
...
/// Inverse transverse-Mercator (UTM north -> WGS84 lon/lat) for an arbitrary
/// northern-hemisphere zone. Only the central meridian depends on the zone.
pub fn utm_to_lonlat(easting: f64, northing: f64, zone: f64) -> (f64, f64) {
...
/// SF (UTM zone 10N) convenience wrapper.
pub fn utm10n_to_lonlat(easting: f64, northing: f64) -> (f64, f64) {
    utm_to_lonlat(easting, northing, 10.0)
}
