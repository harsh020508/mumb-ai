...
- **Map Tile Database Reproducibility:** Pre-rendered SQLite spatial map tiles (`server_tiles/*.db`) committed and unignored in git for clean, reproducible Docker/CI builds across all 5 Indian cities. Each DB's `meta.manifest` bbox must sit inside that city's real WGS-84 envelope (`scripts/verify_integration.py` step 7). SF copies fail this check.
